...
async def test_all_four_tools_are_registered(queries: QueryService) -> None:
    tools = await build_mcp_server(queries).list_tools()
...
async def test_tool_descriptions_carry_the_vocabulary(queries: QueryService) -> None:
    ...
    tools = {t.name: t for t in await build_mcp_server(queries).list_tools()}
...
async def test_search_code_dispatches_and_excludes_tests(queries: QueryService) -> None:
    result = await build_mcp_server(queries).call_tool(
        "search_code", {"query": "store", "limit": 10}
    )
...
async def test_unknown_doc_type_is_an_error_not_an_empty_list(
    ...
    with pytest.raises(ToolError) as caught:
        await build_mcp_server(queries).call_tool(
            "find_guidance", {"query": "structure", "doc_type": "blueprint"}
        )
...
async def test_an_alias_is_not_an_error(queries: QueryService) -> None:
    result = await build_mcp_server(queries).call_tool(
        "find_guidance", {"query": "structure", "doc_type": "spec"}
    )
    assert getattr(result, "isError", False) is False


async def test_taxonomy_resource_is_served_as_json(queries: QueryService) -> None:
    server = build_mcp_server(queries)
...
async def test_resource_and_tool_agree(queries: QueryService) -> None:
